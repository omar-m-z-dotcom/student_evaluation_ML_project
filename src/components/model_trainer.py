...
class ModelTrainer:
    ...
    def initiate_model_trainer(self,train_array:np.ndarray,test_array:np.ndarray):
        ...
        try:
            ...
            best_model_test_score = max([evaluation_results[model][1] for model in evaluation_results.keys()])
            if best_model_test_score <= 0.6:
                logging.info("No good model found with test R2 score more than 0.6")
                raise CustomException("No good model found with test R2 score more than 0.6")
            ...
            for model in evaluation_results.keys():
                if evaluation_results[model][1] == best_model_test_score:
                    ...
                    logging.info(f"Best model found: {model} with test R2 score: {best_model_test_score} and train R2 score: {best_model_train_score}")
                    break
